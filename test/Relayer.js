...
describe.only('Relayer Tests', function () {
    ...
    describe('Relaying trasnaction', function () {
        ...
        it.only('Should create smart wallet for new user and relay transaction', async () => {
            ...
            let setData = smartWalletInterface.functions.addGuardian.encode([bobAccount.signer.address, guardianData.dataHash, guardianData.dataSig]);
            const setSignature = await generateExecutionSignature(aliceAccount.signer, setData, 0, 0, accountAddress, nonce)

            metaTxBatchData = smartWalletInterface.functions.execute.encode([
        ...
        it('Should relay next data transaction successfully', async () => {
            ...
            let metaTxBatchData = await smartWalletInstance.contract.interface.functions.execute.encode([
